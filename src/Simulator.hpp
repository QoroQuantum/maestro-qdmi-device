...
public:
    ...
    {
        if (handle)
            return MaestroLibrary::RemoveAllOptimizationSimulatorsAndAdd(handle, simType,
                                                                         simExecType) == 1;
...
public:
    ...
    void FreeDoubleVector(double* vec) override { MaestroLibrary::FreeDoubleVector(vec); }
    ...
    {
        if (simulatorPtr)
            return MaestroLibrary::SampleCounts(simulatorPtr, qubits, nrQubits, shots);
        return nullptr;
    }
    ...
    {
        if (simulatorPtr)
            return MaestroLibrary::ApplyU(simulatorPtr, qubit, theta, phi, lambda, gamma);
        return 0;
    }
    ...
    {
        if (simulatorPtr)
            return MaestroLibrary::ApplyCSXDG(simulatorPtr, controlQubit, targetQubit);
        return 0;
    }
    ...
    {
        if (simulatorPtr)
            return MaestroLibrary::ApplyCP(simulatorPtr, controlQubit, targetQubit, theta);
        return 0;
    }
    ...
    {
        if (simulatorPtr)
            return MaestroLibrary::ApplyCRx(simulatorPtr, controlQubit, targetQubit, theta);
        return 0;
    }
    ...
    {
        if (simulatorPtr)
            return MaestroLibrary::ApplyCRy(simulatorPtr, controlQubit, targetQubit, theta);
        return 0;
    }
    ...
    {
        if (simulatorPtr)
            return MaestroLibrary::ApplyCRz(simulatorPtr, controlQubit, targetQubit, theta);
        return 0;
    }
    ...
    {
        if (simulatorPtr)
            return MaestroLibrary::ApplyCCX(simulatorPtr, controlQubit1, controlQubit2,
                                            targetQubit);
        return 0;
    }
    ...
    {
        if (simulatorPtr)
            return MaestroLibrary::ApplyCSwap(simulatorPtr, controlQubit, qubit1, qubit2);
        return 0;
    }

    int ApplyCU(int controlQubit, int targetQubit, double theta, double phi, double lambda,
                double gamma)
    {
        if (simulatorPtr)
            return MaestroLibrary::ApplyCU(simulatorPtr, controlQubit, targetQubit, theta, phi,
                                           lambda, gamma);
        return 0;
    }
